...
```
Register → Configure store → Add products → Set up payments → Add domain → Go live
```
...
1. Navigate to the dashboard: `https://admin.example.com/register`
...
```bash
curl -H "Host: acme-store.example.com" \
...
```
https://pay.example.com/api/webhooks/{provider}/{orgId}
```
...
The store is immediately available at:

```
https://{slug}.{NEXT_PUBLIC_PLATFORM_DOMAIN}
→ https://acme-store.example.com
```
