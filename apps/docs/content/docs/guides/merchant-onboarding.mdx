...
1. Navigate to the dashboard: `https://admin.prood.app/register` (or your `NEXT_PUBLIC_DASHBOARD_URL` in dev)
...
curl -H "Host: acme-store.prood.app" \
  https://api.prood.app/v1/products
```
...
```
https://pay.prood.app/api/webhooks/{provider}/{orgId}
```
