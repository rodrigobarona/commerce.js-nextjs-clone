...
Unlike the upstream [Commerce.js Nuxt reference](https://github.com/commerce-js/commerce.js) (where storefronts imported `@prood/core` directly), the Next.js stack uses an **API-centric architecture**: all commerce reads and writes go through `apps/api`, and client applications use `@prood/api-client`. See [Upstream comparison](/docs/architecture/upstream-comparison).
