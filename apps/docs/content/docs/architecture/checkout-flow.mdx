...
Storefront /checkout page
    │
    ├─ Customer fills address + contact info
    │
    ├─ startCheckout() server action
    │     ├─ API: POST /v1/carts/{id}/place-order  → creates order
    │     └─ Checkout: POST /api/sessions           → creates Redis session
    │
    ├─ Redirect to checkout /pay/{sessionId}
    │     ├─ loadSession() from Redis
    │     ├─ Rebuild PaymentProvider with tenant credentials
    │     └─ Render Stripe Element or reference payment UI
    │
    ├─ Customer pays
    │     ├─ POST /api/sessions/{id}/pay
    │     └─ Provider creates charge / payment intent
    │
    ├─ 3DS redirect (if required)
    │     └─ POST /api/sessions/{id}/confirm
    │
    ├─ Webhook (async safety net)
    │     └─ POST /api/webhooks/{provider}/{org}
    │           └─ forwardPaymentWebhook() → API
    │
