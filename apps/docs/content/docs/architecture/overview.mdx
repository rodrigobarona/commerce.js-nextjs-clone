...
│                           Client surfaces                                │
├──────────────┬──────────────┬──────────────┬────────────────────────────┤
│  Storefront  │  Dashboard   │   Checkout   │  Agents / MCP / API keys   │
│   :3000      │   :3002      │   :3004      │                            │
└──────┬───────┴──────┬───────┴──────┬───────┴──────────────┬─────────────┘
       │              │              │                      │
       │ @prood/      │ @prood/      │ @prood/              │ Bearer JWT /
       │ api-client   │ api-client   │ checkout-host        │ x-api-key /
       │              │ + commerce*  │                      │ session cookie
       ▼              ▼              ▼                      ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                         apps/api  (:3005)                                │
│  REST /v1/*  ·  OpenAPI  ·  MCP  ·  Agent Auth  ·  Webhook ingress    │
│  resolveCaller() → orgId + scopes → commerce-service.ts                 │
└──────────────────────────────┬──────────────────────────────────────────┘
                               │
                               ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                      @prood/commerce (server-only)                       │
│  catalog · cart · checkout · orders · admin · payments · integrations   │
│  withTenant(orgId) → adapter + provider factory + cache tags            │
└──────────────────────────────┬──────────────────────────────────────────┘
                               │
                               ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                      @prood/platform (Drizzle + Neon)                   │
│  CommerceAdapter · AdminAPI · RLS · seed · migrate                      │
└──────────────────────────────┬──────────────────────────────────────────┘
                               │
                               ▼
                        Neon Postgres
                   (forced RLS per organization_id)
```
...
In the upstream Nuxt reference, storefronts imported `@prood/core` directly. This Next.js stack routes **all commerce operations through `apps/api`**:
...
This mirrors the upstream `hosted-checkout` pattern but uses Next.js + Upstash instead of Nuxt + in-memory sessions.
