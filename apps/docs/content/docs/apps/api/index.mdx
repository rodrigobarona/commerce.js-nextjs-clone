...
HTTP Request
  │
  resolve-caller.ts
  ├─ Bearer JWT → Agent Auth → org + scopes
  ├─ x-api-key → API key metadata → org + scopes
  ├─ Session cookie → activeOrganizationId → admin + storefront scopes
  └─ Host header → tenant domain lookup → storefront scope
  │
  requireCaller(scope)
  │
  commerce-service.ts
  └─ @prood/commerce.withTenant(orgId) → business logic
```
...
| `COMMERCE_ADAPTER` | Adapter backend (`platform`) |
