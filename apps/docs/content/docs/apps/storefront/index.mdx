...
```
Browser
  ├─ Server Components → commerce-data.ts → @prood/api-client → API
  ├─ Client Components → CartProvider → BFF /api/commerce/cart/* → API
  └─ Server Actions → startCheckout() → API + Checkout app
```
