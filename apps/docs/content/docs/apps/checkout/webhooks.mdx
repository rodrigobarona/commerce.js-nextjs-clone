...
```
POST https://pay.example.com/api/webhooks/stripe/org_abc123
POST https://pay.example.com/api/webhooks/easypay/org_abc123
POST https://pay.example.com/api/webhooks/ifthenpay/_
```
...
Payment provider
  ...
  │
  Checkout app
  ├─ Read raw body + signature headers
  │
  @prood/checkout-host forwardPaymentWebhook()
  │
  POST {COMMERCE_API_URL}/webhooks/payments/{provider}?org={org}
  Header: x-checkout-secret: {CHECKOUT_API_SECRET}
  │
  Commerce API
  ├─ verifyPaymentWebhook(payload, signature, provider, org)
  │     └─ Uses tenant's stored webhook secret (or env fallback)
  │
  └─ Update order status (pending → confirmed / failed)
```
...
2. URL: `https://pay.example.com/api/webhooks/stripe/{orgId}`
...
2. URL: `https://pay.example.com/api/webhooks/easypay/{orgId}`
3. Easypay sends payment status notifications

### Ifthenpay

1. Configure callback URL in Ifthenpay backoffice
2. URL: `https://pay.example.com/api/webhooks/ifthenpay/{orgId}`
