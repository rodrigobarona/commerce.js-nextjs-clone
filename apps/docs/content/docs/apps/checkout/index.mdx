...
```
Storefront startCheckout()
  │
  POST /api/sessions  (x-checkout-secret)
  │
  @prood/checkout-host
  ├─ Creates CheckoutSession (state machine)
  ├─ Saves snapshot to Upstash Redis
  └─ Returns { id, payUrl }
  │
  Customer → /pay/{id}
  ├─ loadSession(id) from Redis
  ├─ Rebuild PaymentProvider with tenant credentials
  └─ Render payment UI (Stripe Element or reference)
  │
  Customer pays → POST /api/sessions/{id}/pay
  │
  Webhook → POST /api/webhooks/{provider}/{org}
  └─ forwardPaymentWebhook() → Commerce API
```
