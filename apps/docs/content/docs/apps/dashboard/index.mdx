...
```
Dashboard page
  ├─ Server Component → admin-api.ts → @prood/api-client → API /v1/admin/*
  ├─ Server Action → admin-api.ts → API /v1/admin/*
  └─ Direct DB (integrations/domains only) → @prood/commerce.withTenant()
```
...
| Billing | `/billing` | Placeholder | Future subscription billing |
...
| `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL` | Merchant auth (default `http://localhost:3002`) |
