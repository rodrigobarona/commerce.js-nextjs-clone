...
| Type | Example | Setup |
| --- | --- | --- |
| **Platform subdomain** | `merchant.example.com` | Automatic — based on organization slug + `NEXT_PUBLIC_PLATFORM_DOMAIN` |
| **Custom domain** | `shop.merchant.com` | Manual — add in dashboard, verify DNS, provision on Vercel |

## Subdomain (automatic)

When a merchant registers, their organization gets a slug (e.g. `acme-store`). The storefront is automatically available at:

```
{slug}.{NEXT_PUBLIC_PLATFORM_DOMAIN}
→ acme-store.example.com
```

No dashboard action required — subdomain resolution happens in `apps/storefront/lib/tenant.ts`.
...
Custom domains are not required for local dev. Use:

- `DEFAULT_TENANT_ORG_ID=org_demo` — serves demo store at `localhost:3000`
- `/etc/hosts` entry for subdomain testing: `127.0.0.1 acme.localhost`
