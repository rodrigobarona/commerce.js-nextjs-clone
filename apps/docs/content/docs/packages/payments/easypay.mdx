...
1. `createSession({ method: 'multibanco' })` → creates Easypay charge
2. Reference numbers returned for Multibanco (entity, reference, amount)
3. Customer pays asynchronously
4. Easypay webhook notifies payment completion
5. Order status updated via webhook handler
